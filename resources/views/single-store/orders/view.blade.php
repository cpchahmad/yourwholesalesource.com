...
{{--                                        {{number_format($order->shipping_price,2)}} USD--}}
    ...
    {{--                                        <button class="btn btn-success" data-toggle="modal" data-target="#payment_modal"><i class="fa fa-credit-card"></i> Credit Card Pay</button>--}}
                                            ...
                                            <button class="btn btn-success wallet-pay-button" data-href="{{route('store.order.wallet.pay',$order->id)}}" data-pay=" {{ number_format($order->total_cost + $order->shipping_rate , 2) }}" ><i class="fa fa-wallet"></i> Wallet Pay</button>
                                                        ...
                                                        <div class="block block-rounded block-themed block-transparent mb-0">
                                                            <div class="block-content cst_content_wrapper font-size-sm text-center">
                                                                ...
                                                                <div class="text-center"> <p>
                                                                        Subtotal: {{number_format(($order->total_cost + $order->shipping_rate ),2)}} USD
                                                                        <br>
                                                                        WholeSaleSource Paypal Fee ({{$settings->paypal_percentage}}%): {{number_format($order->total_cost + $order->shipping_rate *$settings->paypal_percentage/100,2)}} USD
                                                                        <br>Total Cost : {{number_format(($order->total_cost + $order->shipping_rate )+(($order->total_cost + $order->shipping_rate)*$settings->paypal_percentage/100),2)}} USD</p>
                                                                </div>
                                                                <p> A amount of  {{number_format(($order->total_cost + $order->shipping_rate ) +(($order->total_cost + $order->shipping_rate)*$settings->paypal_percentage/100),2)}} USD will be deducted through your Paypal Account</p>
                                                ...
                                                paypal.Buttons({
                                                    createOrder: function(data, actions) {
                                                        return actions.order.create({
                                                            purchase_units: [{
                                                                amount: {
                                                                    value: '{{number_format(($order->total_cost + $order->shipping_rate ) +($order->cost_to_pay*$settings->paypal_percentage/100),2)}}'
                                                                }
                                                            }]
                                                        });
                                                    },
